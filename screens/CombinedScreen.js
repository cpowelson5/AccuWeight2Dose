...
export default function CombinedScreen({ navigation }) {
  const [selectedIndex, setSelectedIndex] = useState(0); // Segmented Control 当前选择
  ...
  const handleFinish = () => {
    ...
    if (weight) {
      // 如果输入了 Weight，直接使用
      finalWeight = convertWeightToKg(parseFloat(weight), weightUnit);
    } else if (height) {
      // 如果仅输入了 Height，通过 JSON 表格估算
      const heightCm = convertHeightToCm(parseFloat(height), heightUnit);
      finalWeight = estimateWeightFromHeight(heightCm);
      if (!finalWeight) {
        Alert.alert('Error', 'Unable to estimate weight from height.');
        return;
      }
    } else {
...
const styles = StyleSheet.create({
  ...
  unitSelector: { marginLeft: 10, flex: 0.4 },
